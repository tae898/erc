...
class FaceAlignment:
    def __init__(self, landmarks_type, network_size=NetworkSize.LARGE,
        ...
        if face_detector == 'blazeface':
            from .detection import blazeface
            self.face_detector = blazeface.FaceDetector(
                device=device, verbose=verbose)
    ...
    def get_landmarks_from_batch(self, image_batch, detected_faces=None):
        ...
        # if sum([len(df) for df in detected_faces]) == 0:
        #     print("Warning: No faces were detected.")
        #     return None
        ...
        for i, faces in enumerate(detected_faces):
            ...
            if len(faces) == 0:
                landmark_set = [np.array([]).reshape(
                    0, 68, 2).astype(np.float32)]
            else:
                landmark_set = []
            ...
            for face in faces:
                ...
                scale = (face[2] - face[0] + face[3] - face[1]) / \
                    self.face_detector.reference_scale
                ...
                if self.landmarks_type == LandmarksType._3D:
                    ...
                    pts, pts_img = pts.view(-1, 68, 2) * \
                        4, pts_img.view(-1, 68, 2)
