...
class ErcTextDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, DATASET='MELD', SPLIT='train', speaker_mode='upper',
                 ...
                 ROOT_DIR='/home/tk/repos/erc/multimodal-datasets/', ONLY_UPTO=False, SEED=0):
...
if DATASET not in DATASETS_SUPPORTED:
    raise ValueError(f"{DATASET} is not one of {DATASETS_SUPPORTED}")
...
OUTPUT_DIR = f"huggingface-results/{DATASET}/{CURRENT_TIME}"
...
model_checkpoint = 'roberta-base'
ROOT_DIR = '/home/tk/repos/erc/multimodal-datasets/'
...
FP16 = True
WARMUP_RATIO = 0.2
emotion2id = get_emotion2id(DATASET)
...
def get_class_weights(DATASET):
    ...
    paths = glob(f'./multimodal-datasets/{DATASET}/raw-texts/*/*.json')
    for path in paths:
        with open(path, 'r') as stream:
            foo = json.load(stream)
        if foo['Emotion'] not in list(emotion2id.keys()):
            continue
        label = emotion2id[foo['Emotion']]
        distributions[DATASET].append(label)

    labels_dict = dict(Counter(distributions[DATASET]))
...
best_run = trainer.hyperparameter_search(
    direction="minimize", hp_space=my_hp_space)
